...
def extraction_on_dataset(model, data_loader, evaluator, flip_test=False, latent_dir=None):
    """
    Extract features with model on the data_loader and evaluate the metrics with evaluator.
    The model will be used in eval mode.
    Args:
        model (nn.Module): a module which accepts an object from
            `data_loader` and returns some outputs. It will be temporarily set to `eval` mode.
    ...
    num_devices = comm.get_world_size()
    logger = logging.getLogger(__name__)
    logger.info("Start extraction&inference on {} images".format(len(data_loader.dataset)))
